...
# Load data ------------------------
load(here::here('GPS Tag/Manuscipt/Data/FastGPS_data_updated.RData'))
...
LS.all.data <-   snaps %>% 
    summarise(N = n(),
            mean.distance = mean(LS.dist.UTM),
            median.distance = median(LS.dist.UTM),
            sd.distance = sd(LS.dist.UTM),
            min.distance = min(LS.dist.UTM),
            max.distance = max(LS.dist.UTM))   
```     
...
ls.time.p <- snaps %>% 
  filter(LS.Quality == "Good") %>% 
  ggplot() +
  geom_point(aes(x=Tdiff.Tag.Track,y=LS.dist.UTM,color=PTT)) +
  scale_color_manual(values=c('lightblue3','lightcoral','darkseagreen3')) +
...
LS.quality.p <- snaps %>% 
  ggplot() +
  geom_point(aes(x=LocSolve.Residual,y=LS.dist.UTM,color=LS.Quality,size=SatsUsed),alpha=0.6) +
  scale_color_manual(values=c('lightblue3','lightcoral')) +
  labs(y = '',x='Residual') +
...
LS.hist.p <- snaps %>% 
  ggplot(aes(x=LS.dist.UTM,fill=LS.Quality)) +
  geom_histogram(color='black') +
  scale_fill_manual(values=c('lightblue3','lightcoral')) +
  labs(x = 'Distance between tag & dolphin (m)')+
  ggtitle("New LocSolve") +
...
----------------------
## FastGPS Analysis - Bench Test
Before we deployed these tags, we performed a 'bench test' to make sure the tags were functioning properly, whereby we activated them and placed them on a boat behind our offices for several hours. We can use the results of this test to compare how the different LocSolve versions performed in estimating the tag's true location (27.335677, -82.576490). For this comparison, I excluded all poor quality data ($\le$ 4 satellites used and residuals > 35) and calculated the distance between the tag's true location and respective estimated locations using the same methods as described above. 


```{r}
# Data Setup ------------------------
bench <- read_csv(here::here('GPS Tag/Manuscipt/Data/BenchTestData.csv'))
...
```{r}
# Data Setup ------------------------
snaps <- read_csv(here::here('GPS Tag/Manuscipt/Data/Argos_snaps.csv'))
```
